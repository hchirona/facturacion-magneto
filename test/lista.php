...
SELECT 
	...
	WHERE sfoi.order_id = sfo.entity_id and (sfo.state NOT IN ("canceled",  "closed") OR sfo.status NOT IN ("canceled",  "closed"))
...
, sum(facturas.iva0 + facturas.iva4 + facturas.iva10 + facturas.iva21) + sum(facturas.bi0 + facturas.bi4 + facturas.bi10 + facturas.bi21) as total 
 from ( 
	...
	,sum(if(sfoi.tax_percent=21,sfoi.tax_amount,0)) +  sfo.shipping_tax_amount + ifnull(sfo.cod_tax_amount,0) as iva21 
	from sales_flat_order sfo, sales_flat_order_item sfoi 
	where (sfo.state NOT IN ("canceled",  "closed") OR sfo.status NOT IN ("canceled",  "closed")) and sfoi.order_id = sfo.entity_id  
...
select
	...
	where  sfo.entity_id = sfop.parent_id and (sfo.state NOT IN ("canceled",  "closed") OR sfo.status NOT IN ("canceled",  "closed"))
